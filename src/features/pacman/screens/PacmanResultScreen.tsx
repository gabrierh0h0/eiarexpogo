...
import {
  completePacmanMission,
  CompletePacmanResult,
} from '../services/pacmanService';
import { PACMAN_MISSION_ID } from '../constants/config';
import { PACMAN_SPRITES } from '../constants/sprites';

type Nav = NativeStackNavigationProp<RootStackParamList, 'PacmanResult'>;
type Rt = RouteProp<RootStackParamList, 'PacmanResult'>;

/**
 * Pantalla final del minijuego Pacman.
 *
 * Flujo idéntico al FoodDropResultScreen:
 *   1. Llega con `score` y `endReason` del engine.
 *   2. Llama al backend para sumar el score.
 *   3. Muestra resultado y botón para volver al Home.
 */
export default function PacmanResultScreen() {
  ...
  const [result, setResult] = useState<CompletePacmanResult | null>(null);

  const submit = async () => {
    setLoading(true);
    setError(null);
    try {
      const r = await completePacmanMission(PACMAN_MISSION_ID, score);
      setResult(r);
    } catch (e: any) {
      const msg =
  ...
  const reasonText =
    endReason === 'lives'
      ? '¡Pascal perdió todas sus vidas!'
      : endReason === 'win'
      ? '¡Recogiste todos los puntos del campus!'
      : '¡Se acabó el tiempo!';

  const titleText =
    endReason === 'win' ? '¡VICTORIA!' : '¡PARTIDA TERMINADA!';
          ...
          <View style={styles.statusBlock}>
            {result.alreadyCompleted ? (
              <Text style={styles.alreadyText}>
                Ya habías completado esta misión antes. Tus puntos no se sumaron
                de nuevo.
              </Text>
            ) : (
              <>
                <Text style={styles.totalLabel}>
                  Tus puntos totales ahora son
                </Text>
